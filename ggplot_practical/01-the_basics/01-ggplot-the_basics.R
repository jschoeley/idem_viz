...
#'
#' Today we shall recreate the above chart and in doing so learn the basics
#' of ggplot. Key concepts we will tackle are *aesthetics*, *layers*, *scales*,
#' and *facets*.

#' The command `library(tidyverse)` loads ggplot plus other packages for reading
#' data, transforming data and reshaping data. Look [here](http://tidyverse.org/)
#' for more information on the tidyverse -- a collection of packages with a
#' common design philosophy.

library(tidyverse)

#' Each ggplot starts with *data*. **Data must be a data frame**! Today we
#' will use data which comes in form of an R package: The gapminder data.
...
#' `aes` stands for *aesthetics*: Mappings between variables in our data and
...
#' `y`-position to the variable `weight`. **Every time you map a variable to a
#' visual property you do it inside `aes()`**
#'
#' The plot looks strange. A single line is drawn across all data points. We
#' want separate lines -- one time series per country...

ggplot(data = gapminder) +
  geom_line(aes(x = year, y = lifeExp, group = country))

#' Note that we don't write `x = gapminder$year` or `y = "lifeExp"`. We simply
#' spell out the name of the variable we wish to work with. ggplot is aware of
#' the dataset we work with -- it is *attached*. Quoting the variable names
#' would actually produce unexpected results:
...
#' unquoted column names to address the variables in your data.**
#'
...
#' variable in the data frame. For comparison, let's move the colour
...
#' same behaviour, but this time it makes sense as we map colour to an actual
...
#' Note that we did not need to specify the `group` argument in the last ggplot
#' call. The have mapped colour to country and therefore implicitly specified
#' the grouping structure of our data. We use `group` only if ggplot fails to
#' correctly guess the grouping.
...
#' `scale_<name of aesthetic>_<continuous|discrete|specialized>`.
#'
...
#' sequence of occurrence in the data.
levels(foo$sex)
...
#' - [THE ggplot2 Cheat-Sheet](https://www.rstudio.com/wp-content/uploads/2016/11/ggplot2-cheatsheet-2.1.pdf).
...
#'   contains all the information about different scales and their options along
...
#'   The book by the author of `ggplot` himself is a good place to learn
#'   the general idea as well as the deeper functionalities of the library.
